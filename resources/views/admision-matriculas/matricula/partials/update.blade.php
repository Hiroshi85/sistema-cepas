{{-- Matrícula edit--}}
<x-modal-edit :id="$matricula->idmatricula" :entity="'Matricula'">
    ...
    <div
        ...
        <!--Modal title-->
        <span class="text-xl mx-auto uppercase dark:text-white">Actualizar matrícula</span>
        <!--Close button-->
        <button
        type="button"
    ...
    <form  method="POST" action="{{ route('matricula.update',$matricula->idmatricula) }}">
        ...
        {{-- <div class="mx-4 my-2">
            ...
            <x-text-input id="año" class="block mt-1 w-full" type="text" name="año" :value="$matricula->año"/>
        ...
        <div class="mx-4 mb-2 flex gap-2">
            <div class="w-[50%]">
                ...
                <x-text-input id="fecha_apertura" class="block mt-1 w-full" type="date" name="fecha_apertura" required autofocus :value="$matricula->fecha_apertura"/>
                <x-input-error :messages="$errors->get('fecha_apertura')" class="mt-2" />
            </div>
            <div class="w-[50%]">
                <x-input-label for="fecha_cierre" :value="__('Fecha de cierre')" />
                <x-text-input id="fecha_cierre" class="block mt-1 w-full" type="date" name="fecha_cierre" required autofocus :value="$matricula->fecha_cierre"/>
        ...
        <div class="mx-4 mb-2 flex gap-2">
            <div class="w-[50%]">
                ...
                <x-text-input id="tarifa" class="block mt-1 w-full dark:text-white" type="text" name="tarifa" required autofocus :value="$matricula->tarifa"/>
                <x-input-error :messages="$errors->get('tarifa')" class="mt-2" />
                </div>
            <div class="w-[50%]">
            <x-input-label for="estado" :value="__('Estado')" class="mb-1"/>
            {{-- <x-text-input id="estado" class="block mt-1 w-full dark:text-white" type="text" name="estado" required autofocus :value="$matricula->estado"/> --}}
                <select name="estado" id="estado" data-te-select-init>
                    <option 
                        @if ($matricula->estado == "Aperturada")
                            selected
                        @endif
