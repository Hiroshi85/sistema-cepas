...
<body class="container my-4">
    ...
    <main>
        @php
            $i = 1
        @endphp
        <h2 style="text-align: center; font-size: 1.5rem">Lista de estudiantes - {{$aula->grado.' '.$aula->seccion}}, año {{date('Y')}}</h2>
        <table style="margin-top: 2px">
            ...
            @foreach ($alumnos as $alumno)
                <tr>
                    <td>{{$i++}}</td>
                    <td colspan="2">{{$alumno->nombre_apellidos}}</td>
                    <td>{{$alumno->numero_celular}}</td>
                    <td class="
                        @if ($alumno->estado == "Matrícula pendiente") text-danger @endif">
                        {{$alumno->estado}}
                    </td>
                </tr>
            @endforeach       
