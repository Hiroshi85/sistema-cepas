<section class="w-full h-full max-h-screen pb-12">
    <div class="px-4 py-4 mb-24">
        ...
        <h1 class="p-2 mb-6 text-xl font-bold dark:text-gray-100">Voucher registrado por {{$voucher->pago->apoderado->nombre_apellidos}}</h1>
         ...
         <form  class="flex flex-col md:flex-row flex-wrap" method="POST" action="{{ route('voucher.update',$voucher->idvoucher) }}" enctype="multipart/form-data">
            ...
            @csrf
            <!--Modal body-->
                <div class="w-full p-4 md:basis-1/2">
                    <img src="{{ asset($selectedNotification->data['voucher']['voucher']) }}" alt="document" class="mx-auto border dark:border-neutral-700 min-w-[40vh] max-w-[50%] md:max-w-[50%] h-auto">
                </div>
                ...
                <div class="mt-4 basis-1 px-2">
                    ...
                    <textarea 
                        ...
                        class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" rows="2">{{$voucher->observacion}}</textarea>    
