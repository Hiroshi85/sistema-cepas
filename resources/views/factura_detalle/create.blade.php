            ...
            <div class="w-full xl:w-8/12 mb-12 xl:mb-0 px-4 mx-auto mt-10">
              <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded ">
                ...
                <div class="block w-full overflow-x-auto">
                    <form action="{{ route('factura_detalle.store',$factura->factura_id) }}" method="POST">
                        @csrf
                        <div class="flex flex-wrap">
                            ...
                            <div class="w-full px-4">
                                <div class="relative w-full mb-3">
                                    ...
                                    <input type="number" name="cantidad" id="cantidad" placeholder="Cantidad" value="{{old('cantidad')}}" class="border-0 px-3 py-3 bg-gray-100  rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                                    @error('cantidad')
                                        <p class="text-red-500 text-xs italic">{{$message}}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="w-full px-4">
                                <div class="relative w-full mb-3">
                                    ...
                                    <input type="number" name="precio" id="precio" placeholder="Precio" value="{{old('precio')}}" class="border-0 px-3 py-3 bg-gray-100  rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                                    @error('precio')
                                        <p class="text-red-500 text-xs italic">{{$message}}</p>
                                    @enderror
                                </div>
