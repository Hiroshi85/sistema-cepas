<div class="flex flex-col gap-5 w-full">
    <div class="flex py-4 w-full items-center justify-between  dark:border-gray-700 border-b border-gray-200 ">
        @livewire('common.search-box', ['placeholder' => 'Ingrese nombre del equipo'])
                    ...
                    <tr
                        ...
                        <td class="px-6 py-4 text-right inline-flex gap-2 items-center justify-center">
                            <a href="{{ route('equipos.edit', $equipo->id) }}"
                                ...
                                @livewire('icons.edit', [], key('edit-icon-' . $equipo->id))
                            </a>
                            {{-- open modal to delete --}}
                            <button wire:click="confirmEquipoDeletion({{ $equipo }})"
                                class="font-medium text-red-600 dark:text-red-500 hover:underline">
                                @livewire('icons.drop', [], key('drop-icon-' . $equipo->id))
                            </button>
    ...
    @if ($confirmingEquipoDeletion)
        <x-confirm-deletion-modal action="{{ route('equipos.destroy', $selectedEquipo) }}"
            title="Estas seguro de querer eliminar este equipo?" confirmButtonText="Sí, eliminar" onClose="cerrarModal"
            cancelButtonText="No, cancelar" wire:click="deleteEquipo">
            <x-slot name="description">
                <p class="mb-3">

                    <span class="font-semibold text-gray-500 dark:text-gray-400">Nombre:</span>
                    <span class="text-gray-500 dark:text-gray-400">{{ $selectedEquipo->nombre }}</span>
                </p>
            </x-slot>
